...
interfaces = config["default"].get("interfaces").replace("_", ".").split(" ") if config["default"].get("interfaces") is not None else []
suppress_mac = config["default"].get("suppress_mac").split(" ") if config["default"].get("suppress_mac") is not None else []
...
def main():
    while True:
        ...
        if protocols == "all" or protocols == "ipv4_only":
            ...
            current_ipv4_entries = [entries for entries in current_ipv4_entries if entries]
            ...
            if len(current_ipv4_entries) > 0:
                for entry in current_ipv4_entries:
                    ...
                    current_entries_dict[mac] = {"ipv4": ipv4, "ipv6": "<unknown>", "interface": interface}
        ...
        if protocols == "all" or protocols == "ipv6_only":
            ...
            # Filter out empty strings
            current_ipv6_entries = [entries for entries in current_ipv6_entries if entries]
            ...
            if len(current_ipv6_entries) > 0:
                for entry in current_ipv6_entries:
                    ...
                    if mac in current_entries_dict:
                        ...
                        current_entries_dict[mac] = {"ipv4": "<unknown>", "ipv6": ipv6, "interface": interface}

        # Delete expired ARP entries
        cursor.execute(f"DELETE FROM arp_entries WHERE timestamp < datetime('now', '-{retention_days} day')")

        # Select all current ARP entries
        cursor.execute("SELECT mac, ipv4, ipv6, interface, hostname, timestamp FROM arp_entries")
        saved_ipv4_entries = set(cursor.fetchall())

        # Save entries in a dictionary with MAC as the key
        saved_entries_dict_by_mac = {
            entry[0]: {"ipv4": entry[1], "ipv6": entry[2], "interface": entry[3], "hostname": entry[4], "timestamp": entry[5]} for entry in saved_ipv4_entries
        }

        # Save entries in a dictionary with MAC as the key
        saved_ipv4_entries_dict_by_ip = {entry[1]: {"mac": entry[0], "interface": entry[3], "hostname": entry[4], "timestamp": entry[5]} for entry in saved_ipv4_entries}

        # Save entries in a dictionary with MAC as the key
        saved_ipv6_entries_dict_by_ip = {entry[2]: {"mac": entry[0], "interface": entry[3], "hostname": entry[4], "timestamp": entry[5]} for entry in saved_ipv4_entries}
        ...
        if len(current_entries_dict) > 0:
            for mac, entry in current_entries_dict.items():
                ...
                hostname = (
                    subprocess.run(
                        "host " + ipv4 + " | grep -v 'not found' | awk '{print $5}' | sed 's/\\.$//'",
                        shell=True,
                        capture_output=True,
                        text=True,
                    )
                ...
                hostname += (
                    subprocess.run(
                        "host " + ipv6 + " | grep -v 'not found' | awk '{print $5}' | sed 's/\\.$//'",
                        shell=True,
                        capture_output=True,
                        text=True,
                    )
                ...
                if mac in saved_entries_dict_by_mac:
                    ...
                    if (protocols == "all" or protocols == "ipv4_only") and log_ipv4_changes and ipv4 != saved_entries_dict_by_mac[mac]["ipv4"]:
                        changes = True
                        changes_message.append(f"OLD IPv4: {saved_entries_dict_by_mac[mac]['ipv4']}")

                    # Check if the IPv6 address has changed
                    if (protocols == "all" or protocols == "ipv6_only") and log_ipv6_changes and ipv6 != saved_entries_dict_by_mac[mac]["ipv6"]:
                        changes = True
                        changes_message.append(f"OLD IPv6: {saved_entries_dict_by_mac[mac]['ipv6']}")

                    # Check if the hostname has changed
                    if log_hostname_changes and hostname != saved_entries_dict_by_mac[mac]["hostname"]:
                        changes = True
                        changes_message.append(f"OLD Hostname: {saved_entries_dict_by_mac[mac]['hostname']}")

                    # Check if the interface has changed
                    # should not happen except, for firewall config changes or if the device has a static IP
                    if log_interface_changes and interface != saved_entries_dict_by_mac[mac]["interface"]:
                        changes = True
                        changes_message.append(f"OLD Interface: {saved_entries_dict_by_mac[mac]['interface']}")

                    # Update the entry
                    if changes:
                        cursor.execute(
                            "UPDATE arp_entries SET ipv4 = ?, ipv6 = ?, interface = ?, hostname = ?, timestamp = ? WHERE mac = ?", (ipv4, ipv6, interface, hostname, time_now, mac)
                        )
                        logging.info(
                            ...
                            + (" | " + " | ".join(changes_message) if len(changes_message) > 0 else "")
                        )
                    # Update the timestamp
                    else:
                        cursor.execute("UPDATE arp_entries SET timestamp = ? WHERE ipv4 = ?", (time_now, ipv4))
                ...
                # This check allows to see, if an address is spoofed or multiple devices have the same IP
                    ...
                    if log_mac_changes and mac != saved_ipv4_entries_dict_by_ip[ipv4]["mac"]:
                        changes = True
                    ...
                    # Check if the hostname has changed
                    if log_hostname_changes and hostname != saved_ipv4_entries_dict_by_ip[ipv4]["hostname"]:
                        changes = True
                        changes_message.append(f"OLD Hostname: {saved_ipv4_entries_dict_by_ip[ipv4]['hostname']}")

                    # Check if the interface has changed
                    # should not happen except, for firewall config changes or if the device has a static IP
                    if log_interface_changes and interface != saved_ipv4_entries_dict_by_ip[ipv4]["interface"]:
                        changes = True
                        changes_message.append(f"OLD Interface: {saved_ipv4_entries_dict_by_ip[ipv4]['interface']}")
                ...
                # This check allows to see, if an address is spoofed or multiple devices have the same IP
                    ...
                    # Check if the MAC address has changed
                    if log_mac_changes and mac != saved_ipv6_entries_dict_by_ip[ipv6]["mac"]:
                        changes = True
                        changes_message.append(
                    ...
                    # Check if the hostname has changed
                    if log_hostname_changes and hostname != saved_ipv6_entries_dict_by_ip[ipv6]["hostname"]:
                        changes = True
                        changes_message.append(f"OLD Hostname: {saved_ipv6_entries_dict_by_ip[ipv6]['hostname']}")

                    # Check if the interface has changed
                    # should not happen except, for firewall config changes or if the device has a static IP
                    if log_interface_changes and interface != saved_ipv6_entries_dict_by_ip[ipv6]["interface"]:
                        changes = True
                        changes_message.append(f"OLD Interface: {saved_ipv6_entries_dict_by_ip[ipv6]['interface']}")
                    ...
                    # Update the timestamp
                        ...
                        cursor.execute("UPDATE arp_entries SET timestamp = ? WHERE ipv6 = ?", (time_now, ipv6))
                    ...
                    cursor.execute(
                        "INSERT INTO arp_entries (mac, ipv4, ipv6, interface, hostname, timestamp) VALUES (?, ?, ?, ?, ?, ?)", (mac, ipv4, ipv6, interface, hostname, time_now)
                    )
...
def mac_vendor_list_download():
    ...
    if not os.path.exists(MAC_VENDOR_FILE) or (time.time() - os.path.getmtime(MAC_VENDOR_FILE)) > 365 * 86400:
...
def mac_vendor_check(mac):
    ...
    for length in range(6, 9):
        matches = [vendor for prefix, vendor in mac_vendor.items() if mac.replace(":", "").lower().startswith(prefix[:length])]
        if len(matches) == 1:
            # Release memory
            del mac_vendor
            gc.collect()
